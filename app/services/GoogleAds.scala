...
class GoogleAds @Inject()(appConfig: AppConfig) {
  ...
  def addCampaignBudget(amount: Int, name: String)(implicit googleAdsClient: GoogleAdsClient, customerId: Long): Either[Throwable, String] =
    Try {
  ...
  def addCampaign(budgetResourceName: String, name: String)(implicit googleAdsClient: GoogleAdsClient, customerId: Long): Either[Throwable, String] =
    Try {
      val startTime = System.currentTimeMillis()
  ...
  def addAdGroups(campaignResourceName: String, products: List[Product])(implicit googleAdsClient: GoogleAdsClient, customerId: Long): Either[Throwable, List[String]] =
    Try {
      val startTime = System.currentTimeMillis()
  ...
  def addKeywords(productsWithAdGroups: List[(Product, String)])(implicit googleAdsClient: GoogleAdsClient, customerId: Long): Either[Throwable, List[String]] =
    Try {
      val startTime = System.currentTimeMillis()
  ...
  def addExpandedTextAds(productsWithAdGroups: List[(Product, String)])(implicit googleAdsClient: GoogleAdsClient, customerId: Long): Either[Throwable, List[String]] =
    Try {
      val startTime = System.currentTimeMillis()
      val operations = for ((product, adGroupResourceName) <- productsWithAdGroups) yield {
